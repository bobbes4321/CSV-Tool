...
CsvTool.Schema
  -> explicit schema DTOs and resolution for presentation and references
```
...
Unity-free and holds optional typed/reference metadata. `CsvTool.Editor`
...
the core document for undo/redo and atomic saves. Dataset validation,
read-only row/column enforcement, and the separate full-record mode are
intentionally excluded; the quick inspector remains available alongside the
grid.
